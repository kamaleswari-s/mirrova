...
export default function RealityCheck() {
  ...
  return (
    <div style={{ padding: isMobile ? '20px 16px' : '40px 48px', maxWidth: 900, color: c.text }}>

      {/* Header */}
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: isMobile ? 26 : 32, color: c.text, margin: '0 0 8px', letterSpacing: '-0.02em' }}>
          Reality Check
        </h1>
      ...
      {!data ? (
        <div style={{ textAlign: 'center', padding: isMobile ? '40px 0' : '80px 0' }}>
          ...
          <p style={{ fontFamily: 'Inter', fontSize: isMobile ? 14 : 15, color: c.textMuted, margin: '0 auto 32px', maxWidth: 480, lineHeight: 1.7, fontWeight: 500 }}>
            We'll analyze your profile against real career data and tell you exactly where you stand — no sugar coating.
          </p>
          <button onClick={generate} disabled={loading}
            style={{ fontFamily: 'Inter', fontStyle: 'italic', fontWeight: 700, fontSize: 16, background: '#722F37', color: '#EEFFBB', border: 'none', borderRadius: 99, padding: '14px 40px', cursor: 'pointer', opacity: loading ? 0.7 : 1, width: isMobile ? '100%' : 'auto' }}>
        ...
        <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 20 : 32 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  <span style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 12, color: scoreColor(data.overall_score), background: `${scoreColor(data.overall_score)}20`, padding: '3px 12px', borderRadius: 99, border: `1px solid ${scoreColor(data.overall_score)}40` }}>
                    {data.score_label || scoreLabel(data.overall_score)}
                  </span>
                </div>
                <p style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: isMobile ? 16 : 20, color: '#F2E8D1', margin: '0 0 8px', lineHeight: 1.3 }}>
                  {data.headline}
                </p>
                <p style={{ fontFamily: 'Inter', fontSize: isMobile ? 12 : 14, color: '#7A6E58', margin: 0, lineHeight: 1.6 }}>
                  {data.market_reality}
                </p>
              </div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 10, marginBottom: 16 }}>
            {data.dimensions?.map((d, i) => (
              <div key={i} style={{ background: c.bgCard, borderRadius: 14, padding: '16px 18px', border: `1px solid ${c.border}` }}>
                ...
                <div style={{ height: 6, background: `${c.accent}15`, borderRadius: 99, marginBottom: 6, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
            <div style={{ background: c.bgCard, borderRadius: 14, padding: '18px', border: `1px solid ${c.border}`, borderTop: '3px solid #722F37' }}>
              ...
              <p style={{ fontFamily: 'Inter', fontSize: 13, color: c.text, margin: 0, lineHeight: 1.6, fontWeight: 500 }}>{data.biggest_gap}</p>
            </div>
            <div style={{ background: c.bgCard, borderRadius: 14, padding: '18px', border: `1px solid ${c.border}`, borderTop: '3px solid #0F9E99' }}>
              ...
              <p style={{ fontFamily: 'Inter', fontSize: 13, color: c.text, margin: 0, lineHeight: 1.6, fontWeight: 500 }}>{data.hidden_strength}</p>
            </div>
            <div style={{ background: '#1A2118', borderRadius: 14, padding: '18px', border: '1px solid rgba(251,160,2,0.2)', borderTop: '3px solid #FBA002' }}>
              ...
              <p style={{ fontFamily: 'Inter', fontSize: 13, color: '#F2E8D1', margin: 0, lineHeight: 1.6, fontWeight: 500 }}>{data.brutal_truth}</p>
            </div>
          </div>
          ...
          <div style={{ background: c.bgCard, borderRadius: 16, padding: isMobile ? '16px' : '24px', marginBottom: 16, border: `1.5px solid ${c.accent}40`, borderLeft: `4px solid ${c.accent}` }}>
            ...
            <p style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: isMobile ? 15 : 17, color: c.text, margin: 0, lineHeight: 1.6 }}>
